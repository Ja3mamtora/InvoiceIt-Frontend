...
import { Plus, Minus, IndianRupee, ArrowLeft, Loader2, AlertCircle, Save } from 'lucide-react';
...
export default function EditQuotation() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const [quotationResponse, customersResponse, productsResponse] = await Promise.all([
          fetch(`${import.meta.env.VITE_BURL}/quotation/${id}`, { credentials: 'include' }),
        ...
        setQuotationItems(quotationData.quotationItems);
  ...
  const addQuotationItem = () => {
    setQuotationItems([...quotationItems, { productId: '', quantity: 1, price: 0, productName: '' }]);
  };
  ...
  const updateQuotationItem = (index, field, value) => {
    ...
    if (field === 'productId') {
      ...
      updatedItems[index].productName = selectedProduct ? selectedProduct.title : '';
    }
    setQuotationItems(updatedItems);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BURL}/editQuotation/${id}`, {
        method: 'PUT',
        headers: {
      ...
      navigate('/quotations', { state: { message: 'Quotation updated successfully!' } });
    } catch (error) {
